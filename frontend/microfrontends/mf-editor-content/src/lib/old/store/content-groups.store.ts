...
import {Fragment, BlockNode, ContentNodesGroup, DragState, FIRST_POSITION, LAST_POSITION} from "./model";
...
export class ContentGroupState {
    cs: ContentServiceIntf;

    constructor(private injector: Injector, private store: Store, private idService: IdService) { //, private co:ContentState

        this.cs = this.injector.get('contentService'); //todo разобраться почему не работает инжектор через конструктор и убрать это
    ...
    async loadVersion({getState, setState}: StateContext<ContentGroupModel>, {
        versionId,
        groupID
    ...
    async saveVersion({getState, setState}: StateContext<ContentGroupModel>, {groupID}: SaveNodesToVersionOfFragment) { //todo стоит добавить fromId для истории
        ...
        if (group) {
            ...
            let articleVersion: Fragment = {articleId: group.fragmentId, blocks: blocks};
            this.cs.newTextVersion(articleVersion).then(frag => {
                ...
                this.store.dispatch(new LoadFragment(group.fragmentId))
